...
@Getter
@Setter
public class Des3SemmetryToJdk implements Semmetry {

    private final Logger logger = LoggerFactory.getLogger(Des3SemmetryToJdk.class);
    ...
    public Key generateKey() {
        try {
            //生成key
            KeyGenerator keyGenerator = KeyGenerator.getInstance("DESede"); //创建密钥生成器
            keyGenerator.init(sp.getDes3KeySize());//初始化生成器
            SecretKey secretKey = keyGenerator.generateKey(); //生成密钥
            byte[] encoded = secretKey.getEncoded();  //保存

            //key转换
            DESedeKeySpec desKeySpec = new DESedeKeySpec(encoded);//创建DES密钥规范
            SecretKeyFactory secretKeyFactory = SecretKeyFactory.getInstance("DESede");//DES密钥的工厂
